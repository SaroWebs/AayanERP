...
import { PurchaseOrder, Department } from '@/types/purchase';
...
export function EditOrderItem({ opened, onClose, order, departments, loading }: Props) {
  ...
  return (
    ...
    >
      ...
      <Modal opened={itemModalOpened} onClose={() => setItemModalOpened(false)} title={editingItemIndex !== null ? 'Edit Item' : 'Add Item'}>
        <Stack>
          <TextInput label="Item Name" value={itemForm.item_name || ''} onChange={e => handleItemFormChange('item_name', e.target.value)} required />
          <NumberInput label="Quantity" value={itemForm.quantity || 1} onChange={value => handleItemFormChange('quantity', value)} min={1} required />
          <TextInput label="Unit" value={itemForm.unit || ''} onChange={e => handleItemFormChange('unit', e.target.value)} />
          ...
          <TextInput label="Total Price" value={itemForm.total_price || 0} readOnly />
